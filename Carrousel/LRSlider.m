...
#define kBackImage(color)  [[LRSlider imageWithColor:(color) size:CGSizeMake(200, 4)] stretchableImageWithLeftCapWidth:100 topCapHeight:2]
#define kThumbImage(color) [LRSlider imageWithColor:(color) size:CGSizeMake(80, 80) radius:40]
...
- (LRSlider *)initWithWidth:(CGFloat)length center:(CGPoint)center horizontal:(BOOL)horizon showValue:(BOOL)show
{
    self = [super init];
    if (self)
    {
...
- (LRSlider *)initWithWidth:(CGFloat)length center:(CGPoint)center horizontal:(BOOL)horizon
{
...
- (void)handleSliderAction:(UIControlEvents)events callback:(LRSliderActionBlock)block
{
...
- (void)removeHandleSliderAction:(UIControlEvents)events
{
    ...
    LRSliderActionBlock block = self.recordDictionary[key];
    if (block != NULL)
    {
    ...
    for (NSNumber *ok in keys)
    {
        ...
        if (evt == 0) continue;
        LRSliderActionBlock block = self.recordDictionary[ok];
...
- (void)callActionBlockWithEvent:(UIControlEvents)event
{
    ...
    LRSliderActionBlock block = [self.recordDictionary objectForKey:thekey];
    if (block)
    {
        block(self, event);
    }
...
- (void)setGradientBackInfo:(NSDictionary *)gradientBackInfo
{
    ...
    CALayer *layer = [LRSlider drawGradientLayerInRect:bounds colors:colors locations:locations startPoint:start endPoint:end];
    _gradientBackInfo = gradientBackInfo;
    [self setBackimage:[LRSlider screenShot:layer]];
}
